			...
			foreach($subscriptions as $subscription) {
				...
				if ($response) {
					...
					if (!empty($subscription['renewed'])) {
						...
						$renewing_sub = $this->recurring_model->GetSubscriptionDetails($subscription['renewed']);
					...
					if ($trip_trigger == TRUE) {
						...
						TriggerTrip('recurring_expire', FALSE, $subscription['subscription_id']);
					}
		...
		if($charges) {
			foreach($charges as $charge) {
				...
				if (!empty($charge['renewed'])) {
					// let's verify this subscription is active
					$renewing_sub = $this->recurring_model->GetSubscriptionDetails($charge['renewed']);
