...
/**
* Topic Relationship Fieldtype
*
* @extends Fieldtype
* @class Relationship_fieldtype
*/

class Topic_relationship_fieldtype extends Fieldtype {
	/**
	* Constructor
	*
	...
	*/
	function __construct () {
		...
		$this->enabled = TRUE;
		$this->fieldtype_name = 'Topic Relationship';
		$this->fieldtype_description = 'Select one or more topics from a list.';
	...
	*/
	function output_shared () {
		...
		// build options
		$this->CI->load->model('publish/topic_model');
		$topics = $this->CI->topic_model->get_tiered_topics();
		...
		if (!empty($topics)) {
			foreach ($topics as $item) {
				$options[] = array('name' => $item['name'], 'value' => $item['id']);
			}
		}
	...
	*/
	function field_form ($edit_id = FALSE) {
	    ...
	    $allow_multiple->label('Allow Multiple Relationships')
			    	  ->name('allow_multiple')
			    	  ->help('If checked, the user can select one or many topics from the list.');
		...
		$default = $this->CI->form_builder->add_field('multiselect');
