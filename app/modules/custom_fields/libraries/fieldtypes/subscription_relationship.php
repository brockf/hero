...
/**
* Subscription Relationship Fieldtype
*
* @extends Fieldtype
* @class Subscription_relationship_fieldtype
*/

class Subscription_relationship_fieldtype extends Fieldtype {
	/**
	* Constructor
	*
	...
	*/
	function __construct () {
		...
		$this->enabled = TRUE;
		$this->fieldtype_name = 'Subscription Plan Relationship';
		$this->fieldtype_description = 'Select one or more subscription plan from a list.';
	...
	*/
	function output_shared () {
		...
		$plans = $this->CI->subscription_plan_model->get_plans();
		...
		if (!empty($plans)) {
			foreach ($plans as $plan) {
				$options[] = array('name' => $plan['name'], 'value' => $plan['id']);
			}
		}
	...
	*/
	function field_form ($edit_id = FALSE) {
	    ...
	    $allow_multiple->label('Allow Multiple Relationships')
			    	  ->name('allow_multiple')
			    	  ->help('If checked, the user can select one or many subscription plans from the list.');
